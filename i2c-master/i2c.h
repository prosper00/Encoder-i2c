...
#define I2C_AFIO_PCFR1 AFIO_PCFR1_I2C1_NOREMAP
#endif
...
#define I2C_STAR1_ERR_MASK ( I2C_STAR1_PECERR | I2C_STAR1_OVR | I2C_STAR1_AF | I2C_STAR1_ARLO | I2C_STAR1_BERR )
...
i2c_result_e i2c_write(
	uint16_t devAddr, uint16_t regAddr, i2c_regAddr_bytes_e regAddrBytes, uint8_t *data, uint8_t sz );
i2c_result_e i2c_read( uint16_t devAddr, uint8_t *data, uint8_t sz );
...
i2c_result_e i2c_read( uint16_t devAddr, uint8_t *data, uint8_t sz )
{
	...
	while ( ( I2C1->STAR2 & I2C_STAR2_BUSY ) && ( timeout-- ) )
		;
	if ( timeout == -1 ) return I2C_TIMEOUT_NOT_BUSY;
	/* Disable the 'write' stuff - a read should be a standalone operation.
	Send a separate i2c_write() if needed.
	...
	*/
	...
	while ( sz-- )
	{
		...
		// while ( i2c_chk_evt( I2C_EVENT_MASTER_BYTE_RECEIVED ) )
		;
